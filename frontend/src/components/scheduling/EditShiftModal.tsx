...
import { generateId, dateToTimestamp, dateTimeToTimestamp, timestampToDateInput, timestampToTimeInput } from '@/lib/utils';
...
// Note: backend doesn't expose updateShift, so we create a new shift record with a new ID
export default function EditShiftModal({ shift, open, onClose, employees }: Props) {
  const [form, setForm] = useState({
    date: timestampToDateInput(shift.date),
    startTime: timestampToTimeInput(shift.startTime),
    endTime: timestampToTimeInput(shift.endTime),
    department: shift.department,
  });
  const [selectedEmployees, setSelectedEmployees] = useState<string[]>([...shift.assignedEmployees]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Shift updated (new entry created).');
      onClose();
    } catch {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="font-display">Edit Shift</DialogTitle>
        </DialogHeader>
                ...
                employees.map((emp) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-sm flex items-center justify-between hover:bg-muted/50 transition-colors ${
                      selectedEmployees.includes(emp.id) ? 'bg-teal-light/30 text-teal-dark' : ''
                    }`}
                  >
